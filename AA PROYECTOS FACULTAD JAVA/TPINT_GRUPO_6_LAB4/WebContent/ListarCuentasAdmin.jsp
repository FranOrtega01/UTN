...
function toggleInput() {
    ...
    if (select.value === "0") {
        ...
        input.disabled = true;     // Deshabilitar la edición del campo de texto
        btnFiltrar.disabled = true; // Deshabilitar el botón Filtrar
    } else {
        input.disabled = false;    // Habilitar la edición del campo de texto
        btnFiltrar.disabled = false; // Habilitar el botón Filtrar
    }
}
    ...
    switch (filtro) {
        ...
        case "Id_cliente":
            if (!/^\d+$/.test(valorFiltro)) {
                crearError("Debe ser un número entero.");
                valido = false;
            }
            break;
        case "Tipo_cuenta":
        ...
        case "Cbu":
            if (!/^\d{22}$/.test(valorFiltro)) {
                crearError("El CBU debe tener exactamente 22 dígitos.");
                valido = false;
            }
            break;
        default:
            crearError("Seleccione un filtro válido.");
            valido = false;
    }
